...
const sampleChildren = [
  { id: "c1", name: "James Wilson" },
  { id: "c2", name: "Emma Thompson" },
  { id: "c3", name: "Michael Brown" },
  { id: "c4", name: "Sophia Davis" },
];
...
const SponsorshipFormModal: React.FC<SponsorshipFormModalProps> = ({
  ...
  // Reset form when modal opens with sponsorship data or as a new sponsorship
  useEffect(() => {
    if (open) {
      if (sponsorship) {
  ...
  const onSubmit = (data: any) => {
    // Validate names based on donor type
    let isValid = true;
    ...
    if (childEntryMode === 'manual') {
      ...
      childName = sampleChildren.find(child => child.id === data.childId)?.name || "";
    }

    // Create a new sponsorship object with all the required fields
    ...
    // If onSave prop is provided, call it with the new sponsorship
    if (onSave) {
